...
import { IStorageUseCase } from "@/core/domain/use-cases";
...
export class StorageS3 implements IStorageUseCase<IGetFeedDto[]> {
  private s3: S3Client;
  ...
  async getFileForFeed() {
    ...
    try {
      ...
      throw new AppError(error?.message ?? "Failed to get file from s3");
    }
  }
